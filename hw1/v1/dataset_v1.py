...
class NerDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.pos_classifier = spacy.load(spacy_path)
        self.pos_classifier.tokenizer = Tokenizer(self.pos_classifier.vocab)
        if train:
            ...
            print('the class is created is test mode : '
    ...
    def create_vocab(self):
        ...
        self.vocab_pos = self.build_vocab_pos()
        print('VOCABS OK')
    ...
    def build_vocab_pos(self):
        data = ['ADJ', 'ADP', 'ADV', 'AUX', 'CCONJ',
                'DET', 'INTJ', 'NOUN', 'NUM', 'PART',
                'PRON', 'PROPN', 'PUNCT', 'SCONJ',
                'SYM', 'VERB', 'X', 'SPACE']
        return MyVocab(data, special_tokens=['<unk>', '<pad>'], from_dict=False)
    ...
    def encode_dataset(self):
        print('START ENCODE DATASET')
        self.encoded_data = list()
        for i in tqdm(range(len(self.window_data))):
            elem = self.window_data[i]
            encoded_x = torch.LongTensor(self.encode_sentence(elem['sentence']))

            pos_x_id = self.encode_pos(elem['sentence'])
            encoded_pos_x = torch.LongTensor(pos_x_id)
            ...
            self.encoded_data.append({"inputs": encoded_x,
                                      "pos_tag": encoded_pos_x,
                                      "outputs": encoded_y})

        print('ENCODE DATASET OK')
    ...
    def encode_pos(self, sentence):
        pos_indices = list()
        sub_sentence = [word for word in sentence if word is not None]
        sub_sentence = ' '.join(sub_sentence)
        sub_sentence = self.pos_classifier(sub_sentence)  # spacyToken
        for word in sub_sentence:
            if word.pos_ in self.vocab_pos.stoi:
                pos_indices.append(self.vocab_pos[word.pos_])
            else:
                pos_indices.append(self.vocab_pos.unk_index)
        for i in range(len(sentence) - len(sub_sentence)):
            pos_indices.append(self.vocab_pos.pad_index)
        return pos_indices

    def encode_test(self, sentence):
        x_id = []
        pos_id = []
        pos_sentence = ' '.join(sentence)
        pos_tag = self.pos_classifier(pos_sentence)
        for token, pos in zip(sentence, pos_tag):
            if token in self.vocab.itos:
                x_id.append(self.vocab[token])
            else :
                x_id.append(self.vocab.unk_index)

            if pos.pos_ in self.vocab_pos.itos:
                pos_id.append(self.vocab_pos[pos.pos_])
            else :
                pos_id.append(self.vocab_pos.unk_index)
        return x_id, pos_id
    ...
    def save_vocabs(self, path='.'):
        ...
        vocab_pos_file = open(os.path.join(path, 'vocab_pos.txt'), 'wb')

        pickle.dump(self.vocab, vocab_file)
        pickle.dump(self.vocab_label, vocab_tag_file)
        pickle.dump(self.vocab_pos, vocab_pos_file)

        vocab_file.close(), vocab_tag_file.close(), vocab_tag_file.close()
    ...
    def load_vocab(self, path_dir='.'):
        ...
        vocab_pos_path = os.path.join(path_dir, 'vocab_pos.txt')


        with open(vocab_path, 'rb') as vocab_file, \
                open(vocab_tag_path, 'rb') as vocab_tag_file, \
                open(vocab_pos_path, 'rb') as vocab_pos_file:
            self.vocab = pickle.load(vocab_file)
            self.vocab_label = pickle.load(vocab_tag_file)
            self.vocab_pos = pickle.load(vocab_pos_file)
